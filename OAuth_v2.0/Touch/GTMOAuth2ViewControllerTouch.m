...
@synthesize request = request_,
            ...
            webView = webView_,
            navigationBarItem = navigationBarItem_,//Added by DAEMMON
            cancelButton = cancelButton_,//Added by DAEMMON
            navigationBar = navigationBar_,//Added by DAEMMON
            leftBarButtonItem = leftBarButtonItem_;//Added by DAEMMON
...
-(void)addActivityObservers{
        [_notificationCenter addObserver:self selector:@selector(gtmAuth2FetchStarted:) name:kGTMOAuth2FetchStarted object:nil];
        [_notificationCenter addObserver:self selector:@selector(gtmAuth2FetchStopped:) name:kGTMOAuth2FetchStopped object:nil];
}
...
-(void)cancelTap{
   ...
   // [[NSNotificationCenter defaultCenter] postNotificationName:kGTMOAuth2TouchCancel object:nil];
    [self dismiss];
}
...
-(void)dismiss{
    [self dismissViewControllerAnimated:YES completion:^{
        ...
        //[[NSNotificationCenter defaultCenter] postNotificationName:kGTMOAuth2DialogDismissed object:nil];
    }];
}
...
- (void)webViewDidStartLoad:(UIWebView *)webView {
    ...
    [_activityDelegate oAuthViewController:self becomeActiveWithMessage:OAVCMessageLoading];
...
-(void)gtmAuth2FetchStarted:(NSNotification *)notification{
    [_activityDelegate oAuthViewController:self becomeActiveWithMessage:OAVCMessageFetchAccess];
        NSLog(@"gtmAuthFetch2Started");
        NSLog(@"%@",notification);
}

-(void)gtmAuth2FetchStopped:(NSNotification *)notification{
    //[_activityDelegate socialAuthVCProviderDoneActivity:self];
    //[notificationCenter removeObserver:self];
    //oauth2IsFetchAccess = NO;
    [_activityDelegate oAuthViewControllerDoneActivity:self];
    NSLog(@"gtmAuthFetch2Stopped");
    NSLog(@"%@",notification);
}


//-(void)gtmAuth2WebViewStoppedLoading:(NSNotification *)notification{
//    [_activityDelegate socialAuthVCProviderDoneActivity:self];
//    if(oauth2IsFetchAccess){
//        [_activityDelegate socialAuthVCProvider:self becomeActiveWithMessage:SAVCMessageFetchAccess];
//    }
//    NSLog(@"gtmAuthWebView2StoppedLoading");
//    NSLog(@"%@",notification);
//}
//-(void)gtmAuth2FetchStarted:(NSNotification *)notification{
//    oauth2IsFetchAccess = YES;
//    NSLog(@"gtmAuthFetch2Started");
//    NSLog(@"%@",notification);
//}
//-(void)gtmAuth2FetchStopped:(NSNotification *)notification{
