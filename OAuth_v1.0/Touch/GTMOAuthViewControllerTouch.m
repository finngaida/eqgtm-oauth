...
@synthesize navigationBarItem = navigationBarItem_;//Added by DAEMMON
@synthesize cancelButton = cancelButton_;//Added by DAEMMON
@synthesize navigationBar = navigationBar_;//Added by DAEMMON
@synthesize leftBarButtonItem = leftBarButtonItem_;//Added by DAEMMON
...
-(void)addActivityObserver{
    [_notificationCenter addObserver:self selector:@selector(gtmAuthFetchStarted:) name:kGTMOAuthFetchStarted object:nil];
    [_notificationCenter addObserver:self selector:@selector(gtmAuthFetchStopped:) name:kGTMOAuthFetchStopped object:nil];
}

- (void)dealloc {
  [navigationBarItem_ release];
...
- (void)viewDidLoad {
  ...
  [cancelButton_ addTarget:self action:@selector(cancelTap) forControlEvents:UIControlEventTouchUpInside];
  [leftBarButtonItem_ setCustomView:cancelButton_];
    
  [rightBarButtonItem_ setCustomView:navButtonsView_];
  [navigationBarItem_ setRightBarButtonItem:rightBarButtonItem_];
  [navigationBarItem_ setLeftBarButtonItem:leftBarButtonItem_];
}

-(void)cancelTap{
    [_activityDelegate oAuthViewControllerDidCancel:self];
    //[_notificationCenter postNotificationName:kGTMOAuthTouchCancel object:nil];
    [self dismiss];
}

-(void)dismiss{
    [self dismissViewControllerAnimated:YES completion:^{
        [_activityDelegate oAuthViewControllerDismissed:self];
        //[_notificationCenter postNotificationName:kGTMOAuthDialogDismissed object:nil];
    }];
}
...
- (void)webViewDidStartLoad:(UIWebView *)webView {
    [_activityDelegate oAuthViewController:self becomeActiveWithMessage:@"webViewDidStartLoad"];
    //[_notificationCenter postNotificationName:kGTMOAuthWebViewStartedLoading object:nil];
  [self updateUI];
}

- (void)webViewDidFinishLoad:(UIWebView *)webView {
    [_activityDelegate oAuthViewControllerDoneActivity:self];
    //[_notificationCenter postNotificationName:kGTMOAuthWebViewStoppedLoading object:nil];
  [self updateUI];
}
//============================DAEMMON COMMENT============================
...
-(void)gtmAuthFetchStarted:(NSNotification *)notification{
    NSLog(@"gtmAuthFetchStarted");
    NSLog(@"%@",notification);
    NSString *fetchType = [notification userInfo][kGTMOAuthFetchTypeKey];
    if([fetchType isEqualToString:kGTMOAuthFetchTypeRequest]){
        //Fetch request token;
        [_activityDelegate oAuthViewController:self becomeActiveWithMessage:OAVCMessageFetchRequest];
        return;
    }
    
    if([fetchType isEqualToString:kGTMOAuthFetchTypeAccess]){
        //Fetch accsess token
        [_activityDelegate oAuthViewController:self becomeActiveWithMessage:OAVCMessageFetchAccess];
        return;
    }
    
    
}
-(void)gtmAuthFetchStopped:(NSNotification *)notification{
    NSLog(@"gtmAuthFetchStopped");
    NSLog(@"%@",notification);
    NSString *fetchType = [notification userInfo][kGTMOAuthFetchTypeKey];
    if([fetchType isEqualToString:kGTMOAuthFetchTypeRequest]){
        //Fetch request token;
